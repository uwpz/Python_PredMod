...
under_samp = Undersample(n_max_per_level = 5000)
...
fit = GridSearchCV(lgbm.LGBMRegressor() if TARGET_TYPE == "REGR" else lgbm.LGBMClassifier(),
                   {"n_estimators": [x for x in range(100, 3100, 500)], "learning_rate": [0.01],
                    "num_leaves": [32,64], "min_child_samples": [10],
                    "colsample_bytree": [0.2], "subsample": [0.7]},
                   cv = split_my1fold_cv.split(df_tune),
                   refit = False,
                   scoring = scoring[TARGET_TYPE],
...
fit = GridSearchCV(xgb.XGBRegressor(verbosity = 0) if TARGET_TYPE == "REGR"
                   else xgb.XGBClassifier(verbosity = 0),
                   param_grid,
